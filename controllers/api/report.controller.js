...
import REPORT_STATE from "../../definitions/constants/REPORT";
import checkStateReport from "../../helper/parse/stateReport";
...
exports.create = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      const time = await Setting.findOne({
        settingName: "Setting time_report",
      });
      // set status_report
      const status_report = await parsestatusreport(
        time.settingValue.start,
        time.settingValue.end
      );

      const state = await checkStateReport(
        time.settingValue.start,
        time.settingValue.end
      );
      const reportState =
        state === true ? REPORT_STATE.REPORTED : REPORT_STATE.REPORTED_LATE;
      ...
      if (list_question) {
        if (list_answer.length != list_question.length) {
          return res.status(500).json({
            error:
              "Please Check answer! List answer not match list question...",
          });
        }
...
exports.get_list_report = async (req, res) => {
  ...
  res.status(202).json({ success: "OK", data: list_question });
};
exports.get_list_reported = async (req, res) => {
  // example: 1597413043000
  ...
  const list_user = await User.aggregate([
    {
      $lookup: {
        from: "reports",
        let: { userId: "$_id" },
            ...
            $match: {
              $expr: {
                $and: [
                  ...
                  {
                    $or: [
                      { $eq: ["$reportState", REPORT_STATE.REPORTED] },
                      { $eq: ["$reportState", REPORT_STATE.REPORTED_LATE] },
                    ],
                  },
                  { $gte: ["$createdAt", currentDate] },
                  { $lt: ["$createdAt", netDate] },
  ...
  const fetchPromise = list_user.map(async (user) => {
    const promises = user.report.map(async (report) => {
      var opts = [{ path: "questionId", select: "_id title" }];
      return Question.populate(report, opts);
    });

    user.report = await Promise.all(promises);
    return user;
  });
  ...
  res.status(202).json({
    success: "OK",
    data: result,
    totalReported: result.length,
  });
};

exports.get_list_report_late = async (req, res) => {
  // example: 1597413043000
      ...
      { $project: { password: 0, createdAt: 0, updatedAt: 0 } },
      {
        $lookup: {
          ...
          pipeline: [
            ...
            { $project: { questionID: 0, _id: 0, userId: 0 } },
          ],
          as: "report",
        },
      },
...
exports.get_list_non_report = async (req, res) => {
  // example: 1597413043000
  ...
  try {
    ...
    res.status(202).json({
      success: "OK",
      data: result,
      totalNonReport: result.length,
    });
  } catch (error) {
    until.handleError(res, error);
  }
};
exports.get_list_users_report = async (req, res) => {
  // example: 1597413043000
